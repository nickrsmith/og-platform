...
contract EmpressaContractFactory {
    ...
    /* fees (bps: 100 = 1%) */
    // Note: New fee structure uses fixed rates in revenue distributor
    // These are kept for backward compatibility but may not be used
    uint32 public empressaFeePct = 200;     // 2% (new default)
    uint32 public integratorFeePct = 0;     // 0% (no longer used, kept for compatibility)
    ...
    /**
     * @notice Update platform fees (kept for backward compatibility)
     * @dev Note: New revenue distributor uses fixed fees (2% Empressa, 0.002% referral)
     *      This function is maintained for compatibility but fees are now managed in revenue distributor
     */
    function updatePlatformFees(uint32 _empressaFeePct, uint32 _integratorFeePct) external onlyAdmin {
        if (_empressaFeePct > 5000) revert EmpressaFeeTooHigh();    // 50% max
        if (_integratorFeePct > 200) revert IntegratorFeeTooHigh(); // 2% max
        ...
        returns (address org)
    {
        ...
        RoleWiringLib.wire(licenseManager, assetRegistry, groupManager, revenueDistributor, org);
    ...
    /**
     * @notice Get platform fees
     * @dev Returns fees in basis points (10000 = 100%)
     *      Note: New revenue distributor uses fixed fees, this is for backward compatibility
     * @return integratorFee Integrator fee (basis points) - now 0% (not used)
     * @return empressaFee Empressa Marketplace LLC fee (basis points) - default 2% (200)
     */

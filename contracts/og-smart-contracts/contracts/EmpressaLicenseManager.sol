...
contract EmpressaLicenseManager is AccessControl, ReentrancyGuard, IEmpressaStructs {
    ...
    function licenseAssetWithDuration(
        ...
        // Get fee percentages from revenue distributor
        (uint32 empressaFeePct, uint32 referralFeePct, , , ) = IEmpressaRevenueDistributor(
            IEmpressaOrgContract(orgContract).revenueDistributor()
        ).getFeePercentages();
        
        // Calculate net seller amount (price minus fees)
        uint256 totalFeePct = empressaFeePct + referralFeePct;
        uint256 netSellerAmount = asset.price * 10000 / (10000 + totalFeePct);
        
        emit LicenseGranted(orgContract, newLicenseId, assetId, licensee, netSellerAmount);
    ...
    function _distributeRevenue(
        ...
        if (revenueDistributor != address(0)) {
            // Get integration partner (can be used as referral wallet)
            address referralWallet = IEmpressaOrgContract(orgContract).integrationPartner();
            
            // Approve revenue distributor to spend the amount
            IERC20(usdcToken).safeApprove(revenueDistributor, amount);
            
            // Call revenue distributor with new signature (includes promoCode, feeMode, buySidePct parameters)
            // For licensing flows, use default: SELL_SIDE_ONLY (seller pays), no promo code
            // Marketplace transactions should call revenue distributor directly with appropriate fee application settings
            IEmpressaRevenueDistributor(revenueDistributor).distributeRevenue(
                ...
                referralWallet,
                orgContract,
                "", // promoCode - empty for licensing flows
                IEmpressaRevenueDistributor.FeeApplication.SELL_SIDE_ONLY, // Default: seller pays
                0   // buySidePct - ignored for SELL_SIDE_ONLY
            );
        } else {
